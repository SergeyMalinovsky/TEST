...
const template = (_id, _state, _text) => {
  ...
  */
    ...
    //console.log('New item is '+check);

    return `<li itemid='${_id}'>
                <div class='item'>
                    <input type='checkbox' ${check} class='ch-box-complete-task${_id}' id='${_id}' onclick='changeStateTask(${_id})'> 
...
function changeStateTask(_id) {
    ...
    for (let i = 0; i < items.length; i++) {
        if (items[i].id == _id) {
            ...
            //console.log("Found!");
            console.log('state-' + items[i].state);
            //break;
        }
    }
    ...
    //let rmText = document.
    if (state_checkbox) {
        document.getElementsByClassName('ch-box-complete-task' + _id)[0].setAttribute('checked', 'true');
        elem.classList.add('cross');
    } else {
        document.getElementsByClassName('ch-box-complete-task' + _id)[0].setAttribute('checked', 'false')
        elem.classList.remove('cross');
    }
}
...
function viewItems() {
    ...
    if (items.length == 0) {
        ...
        items.forEach(i => {
            template_li.innerHTML += template(i.id, i.state, i.text);
            article.append(template_li);
            changeStateLabel(i.id, i.state);
        })
        /*for (let i = 0; i < items.length; i++) {
            //console.log(items.length);
